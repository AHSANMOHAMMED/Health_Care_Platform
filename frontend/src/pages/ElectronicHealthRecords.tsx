    ...
    <div className="min-h-screen bg-slate-50 text-slate-800">
      <header className="fixed top-0 left-0 right-0 z-50 bg-slate-50/90 backdrop-blur-md border-b border-slate-300/30">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold text-slate-900">Health Records</h1>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {[
              ...
              <div key={stat.label} className="bg-white border border-slate-300/50 rounded-xl p-4">
          ...
          <div className="space-y-4">
            {filteredRecords.map(record => (
              <div
                ...
                className="bg-white border border-slate-300/50 rounded-xl p-5 cursor-pointer hover:border-[#0EA5E9]/30 transition-all"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-700 mt-2 line-clamp-2">{record.description}</p>
      ...
      {selectedRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70" onClick={() => setSelectedRecord(null)}>
          <div className="bg-white border border-slate-300/50 rounded-2xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${getTypeColor(selectedRecord.type)}`}>
                {getTypeIcon(selectedRecord.type)}
              </div>
              <div>
                <h3 className="font-semibold text-slate-900">{selectedRecord.title}</h3>
                <p className="text-sm text-slate-600">{selectedRecord.date}</p>
              </div>
            </div>
            <div className="space-y-4">
              <div>
              ...
              {selectedRecord.attachments && selectedRecord.attachments > 0 && (
                <div>
                  <p className="text-sm text-slate-600 mb-2">Attachments</p>
                  <div className="flex gap-2">
                    {Array.from({ length: selectedRecord.attachments }).map((_, i) => (
                      <button key={i} className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-lg text-sm text-slate-700 hover:text-slate-900">
                        <FileText size={16} />
                        File {i + 1}
                      </button>
                    ))}
            ...
            <button
              ...
              className="w-full mt-6 py-3 bg-[#0EA5E9] hover:bg-[#0284C7] text-slate-900 rounded-xl font-medium transition-all"
            >
              Close
            </button>
          </div>
