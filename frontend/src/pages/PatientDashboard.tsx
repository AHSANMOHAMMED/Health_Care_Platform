...
export default function PatientDashboard() {
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><Loader2 className="w-8 h-8 text-[#0EA5E9] animate-spin" /></div>;
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-700">
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="px-4 py-2">
            <div className="clinical-card p-4 mb-6 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900">{(user as any)?.firstName || 'Patient'}</p>
                <p className="text-[10px] text-slate-600">ID: {(user as any)?.id || 'PT-10024'}</p>
              </div>
            </div>
      ...
      <main className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <div className="flex-1 p-4 lg:p-8 overflow-auto">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="clinical-card p-5">
                  ...
                  <p className="text-2xl font-bold text-slate-900">72 <span className="text-sm text-slate-500 font-normal">bpm</span></p>
                  <p className="text-xs text-slate-600 mt-1">Heart Rate</p>
                </div>
                <div className="clinical-card p-5">
                  <div className="flex justify-between items-start mb-2">
                  ...
                  <p className="text-2xl font-bold text-slate-900">120/80 <span className="text-sm text-slate-500 font-normal">mmHg</span></p>
                  <p className="text-xs text-slate-600 mt-1">Blood Pressure</p>
                </div>
                <div className="clinical-card p-5">
                  <div className="flex justify-between items-start mb-2">
                  ...
                  <p className="text-xs text-slate-600 mt-1">Next Appointment</p>
                </div>
              </div>

              <div className="clinical-card p-6">
                <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2"><AlertTriangle size={16} className="text-[#FFBE29]" /> Emergency SOS Protocol</h3>
                <div className="p-4 bg-red-900/10 border border-red-900/30 rounded-xl flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 mb-3">One-tap emergency alert will share your exact GPS location and critical medical history (Blood Type, Allergies) with responders.</p>
                ...
                {prescriptions.map(rx => (
                  <div key={rx.id} className="clinical-card p-5 border-l-4 border-l-[#0EA5E9]">
                    <div className="flex justify-between items-start mb-3">
                      <h3 className="font-bold text-slate-900 flex items-center gap-2"><Pill size={16} className="text-[#0EA5E9]"/> {rx.medicineName}</h3>
          ...
          {activeTab === 'history' && (
            <div className="space-y-6">
              <h2 className="text-lg font-bold text-slate-900">My Medical Profile</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="clinical-card p-6">
                  <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2"><AlertTriangle size={16} className="text-amber-400" /> Allergies & Reactions</h3>
                  <div className="space-y-3">
                    ...
                    <button className="w-full py-2 border border-dashed border-slate-300 rounded-xl text-xs text-slate-500 hover:border-[#0EA5E9] hover:text-[#0EA5E9] transition-all">+ Add Allergy</button>
                  </div>
                </div>
                <div className="clinical-card p-6">
                  <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2"><Activity size={16} className="text-sky-400" /> Chronic Conditions</h3>
                  <div className="space-y-3">
                    ...
                    <button className="w-full py-2 border border-dashed border-slate-300 rounded-xl text-xs text-slate-500 hover:border-[#0EA5E9] hover:text-[#0EA5E9] transition-all">+ Add Condition</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'ai-scans' && (
            <div className="space-y-6">
              <h2 className="text-lg font-bold text-slate-900">AI Analysis History</h2>
                  ...
                  <div key={i} className="clinical-card p-5 hover:border-[#0EA5E9]/30 transition-all cursor-pointer group">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <h4 className="font-bold text-slate-900 group-hover:text-[#0EA5E9] transition-colors">{scan.symptoms}</h4>
                      </div>
                      <span className={`badge-${scan.severity === 'CRITICAL' ? 'danger' : scan.severity === 'HIGH' ? 'warning' : 'success'}`}>{scan.severity}</span>
                    </div>
                    <div className="flex items-center justify-between text-xs">
                      <p className="text-slate-500">Recommended Specialty: <span className="text-slate-700">{scan.specialty}</span></p>
          ...
          {activeTab === 'billing' && (
            <div className="space-y-6">
              <h2 className="text-lg font-bold text-slate-900">Billing & Transactions</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="clinical-card p-5 border-l-4 border-l-emerald-500">
                  ...
                  <p className="text-2xl font-bold text-slate-900">LKR 7,500.00</p>
                </div>
                <div className="clinical-card p-5 border-l-4 border-l-amber-500">
                  ...
                  <p className="text-2xl font-bold text-slate-900">LKR 2,500.00</p>
                </div>
              </div>
              <div className="clinical-card overflow-hidden">
                <table className="clinical-table">
                  ...
                  <tbody>
                    {transactions.map((tx, i) => (
                      <tr key={i}>
                        ...
                        <td className="font-mono text-[10px] text-slate-600">TXN-{tx.id}</td>
                        <td className="font-bold text-slate-900">LKR {tx.amount.toLocaleString()}</td>
