      ...
      <div className="max-w-5xl w-full">
           ...
           {steps.map(s => (
             <div key={s.id} className="relative z-10 flex flex-col items-center">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 border-4 ${
                  step >= s.id ? 'bg-indigo-600 border-indigo-100 text-white shadow-lg shadow-indigo-200' : 'bg-white border-slate-100 text-slate-400'
                }`}>
                   <s.icon size={20} />
                </div>
                <div className="hidden md:block absolute -bottom-8 whitespace-nowrap text-center">
                   <p className={`text-xs font-black uppercase tracking-widest ${step >= s.id ? 'text-indigo-600' : 'text-slate-400'}`}>{s.title}</p>
                </div>
             </div>
           ))}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mt-16">
           ...
           <div className="lg:col-span-1 space-y-6 hidden lg:block">
              <div className="premium-glass p-8 bg-slate-900 text-white relative overflow-hidden h-full">
                 ...
                 <p className="text-slate-400 font-medium leading-relaxed mb-8">Your medical data is encrypted with AES-256 standards and stored in compliance with SL-Health privacy laws.</p>
                 ...
                 <div className="flex-1 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {step === 2 && (
                       <div className="space-y-8">
                          <div>
                             <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Known Allergies</label>
                             <div className="flex flex-wrap gap-2">
                                ...
                                <button className="px-4 py-2 bg-slate-50 text-slate-400 rounded-xl text-sm font-bold border border-dashed border-slate-300 hover:bg-slate-100 transition-colors">
                                   + Add New
                                </button>
                             </div>
                          </div>
                          ...
                          <div>
                             <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Current Medications</label>
                             <div className="space-y-3">
                                ...
                                <button className="w-full py-3 bg-white border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 font-bold hover:bg-slate-50 transition-all">+ Add Medical History Item</button>
                             </div>
                          </div>
                       </div>
                    )}
                    ...
                    {step === 3 && (
                       <div className="grid grid-cols-2 gap-6">
                          <div className="col-span-2">
                             <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Provider Name</label>
                          ...
                          <div className="col-span-2 md:col-span-1">
                             <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Policy Number</label>
                             <input type="text" className="input-premium" placeholder="POL-99281-X" />
                          </div>
                          <div className="col-span-2 md:col-span-1">
                             <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Group ID</label>
                    ...
                    {step === 4 && (
                       <div className="flex flex-col items-center justify-center h-full py-10">
                          <div className="w-full border-4 border-dashed border-slate-100 rounded-[2rem] p-12 text-center group hover:border-indigo-100 transition-all cursor-pointer bg-slate-50/50">
                             ...
                             <button className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold text-sm">Browse Files</button>
                          </div>
                 ...
                 <div className="mt-12 pt-8 border-t border-slate-100 flex justify-between gap-4">
                    ...
                    {step < 4 ? (
                      ...
                      <button 
                        ...
                        disabled={loading}
                        className="px-10 py-4 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-500 transition-all shadow-xl shadow-emerald-500/20 flex items-center gap-2 min-w-[180px] justify-center">
