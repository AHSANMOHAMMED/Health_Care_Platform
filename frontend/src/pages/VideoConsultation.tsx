                ...
                <div className="absolute top-0 w-full p-6 flex justify-between items-start z-10 bg-gradient-to-b from-slate-900/90 to-transparent pointer-events-none">
                    <div>
                        <div className="flex items-center gap-3 bg-slate-900/50 backdrop-blur-md px-4 py-2 rounded-full border border-slate-700 pointer-events-auto">
                           ...
                           <h3 className="text-slate-900 font-bold text-sm">Secure Consultation</h3>
                           <div className="w-px h-4 bg-slate-700 mx-1"></div>
                           <span className="text-slate-600 text-xs font-bold font-mono">00:00</span>
                        </div>
                    </div>
                ...
                <div className="flex-1 w-full h-full relative z-0 bg-slate-900" ref={jitsiContainerRef}>
                    {loading && (
                       <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 z-20">
                          ...
                          <p className="text-slate-700 text-lg font-medium">Establishing E2E Encrypted Connection...</p>
                       </div>
                    )}
                </div>
                ...
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
                   <div className="bg-slate-900/80 backdrop-blur-xl px-8 py-4 rounded-[2rem] border border-slate-700/50 shadow-2xl flex items-center gap-6">
                      <button
                        ...
                        className={`group relative h-14 w-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${isMicOn ? 'bg-slate-800 text-slate-900 hover:bg-slate-700' : 'bg-red-500/20 text-red-500 hover:bg-red-500/30'}`}>
                          {isMicOn ? <Mic size={24} /> : <MicOff size={24} />}
                          <span className="absolute -top-8 bg-slate-800 text-slate-900 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">Audio</span>
                      </button>
                      
                      <button
                        onClick={() => setIsVideoOn(!isVideoOn)}
                        className={`group relative h-14 w-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${isVideoOn ? 'bg-slate-800 text-slate-900 hover:bg-slate-700' : 'bg-red-500/20 text-red-500 hover:bg-red-500/30'}`}>
                          {isVideoOn ? <Video size={24} /> : <VideoOff size={24} />}
                          <span className="absolute -top-8 bg-slate-800 text-slate-900 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">Video</span>
                      </button>

                      <div className="w-px h-8 bg-slate-700"></div>

                      <button className="group relative h-14 w-14 rounded-2xl flex items-center justify-center bg-slate-800 text-slate-700 hover:bg-slate-700 hover:text-slate-900 transition-all duration-300">
                         <Settings size={22} />
                         <span className="absolute -top-8 bg-slate-800 text-slate-900 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">Settings</span>
                      </button>

                      <button
                        onClick={() => navigate('/patient')}
                        className="h-14 px-8 rounded-2xl bg-red-600 hover:bg-red-500 text-slate-900 flex items-center justify-center transition-all duration-300 shadow-lg shadow-red-600/20 gap-3 font-bold group">
