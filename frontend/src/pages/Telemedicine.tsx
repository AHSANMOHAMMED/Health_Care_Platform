  ...
  if (connectionStatus === 'connecting') {
    return (
      <div className="h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        ...
        <h2 className="text-2xl font-black text-white mb-2">Securing Connection</h2>
        <p className="text-slate-400 font-medium">Establishing E2E Encrypted WebRTC Bridge...</p>
      </div>
    );
  }
      ...
      <div className="flex-1 bg-slate-950 rounded-[2.5rem] overflow-hidden flex flex-col relative shadow-2xl border border-slate-800">
        ...
        <div className="absolute top-0 w-full p-6 flex justify-between items-start z-10 bg-gradient-to-b from-slate-900/90 to-transparent">
          <div className="flex items-center gap-3 bg-slate-900/50 backdrop-blur-md px-4 py-2 rounded-full border border-slate-700">
             ...
             <h3 className="text-white font-bold text-sm hidden sm:block">{sessionInfo?.doctorName}</h3>
             <div className="w-px h-4 bg-slate-700 mx-1"></div>
             <span className="text-slate-300 text-xs font-bold font-mono">{formatSessionTime(sessionTime)}</span>
          </div>
          
          <div className="flex gap-2">
            <button onClick={() => setShowSidebar(showSidebar === 'participants' ? null : 'participants')} className={`w-10 h-10 rounded-full backdrop-blur-md border border-slate-700 flex items-center justify-center transition-colors ${showSidebar === 'participants' ? 'bg-indigo-600 text-white' : 'bg-slate-900/50 text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
               <Users size={18} />
            </button>
            <button onClick={() => setShowSidebar(showSidebar === 'chat' ? null : 'chat')} className={`w-10 h-10 rounded-full backdrop-blur-md border border-slate-700 flex items-center justify-center transition-colors ${showSidebar === 'chat' ? 'bg-indigo-600 text-white' : 'bg-slate-900/50 text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
           <div className="bg-slate-900/80 backdrop-blur-xl px-4 md:px-8 py-3 md:py-4 rounded-[2rem] border border-slate-700/50 shadow-2xl flex items-center gap-3 md:gap-6">
              <button onClick={() => setIsMicOn(!isMuted)} className={`group relative h-12 w-12 md:h-14 md:w-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${isMuted ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30' : 'bg-slate-800 text-white hover:bg-slate-700'}`}>
                  {isMuted ? <MicOff size={22} /> : <Mic size={22} />}
              </button>
              
              <button onClick={() => setIsVideoOff(!isVideoOff)} className={`group relative h-12 w-12 md:h-14 md:w-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${isVideoOff ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30' : 'bg-slate-800 text-white hover:bg-slate-700'}`}>
                  {isVideoOff ? <VideoOff size={22} /> : <Video size={22} />}
              </button>

              <div className="w-px h-8 bg-slate-700"></div>

              <button onClick={() => navigate('/patient')} className="h-12 md:h-14 px-6 md:px-8 rounded-2xl bg-red-600 hover:bg-red-500 text-white flex items-center justify-center transition-all duration-300 shadow-lg shadow-red-600/20 gap-2 font-bold group">
           ...
           {showSidebar === 'chat' && (
             <>
               <div className="px-6 py-5 bg-slate-900 border-b border-slate-800 flex justify-between items-center">
                 <h3 className="font-bold text-white flex items-center gap-2"><MessageSquare size={18} className="text-indigo-400"/> Session Chat</h3>
               </div>
               ...
               <div className="flex-1 p-4 overflow-y-auto space-y-4">
                  ...
                  {chatMessages.map(msg => (
                     <div key={msg.id} className={`flex flex-col ${msg.senderName === 'You' || msg.senderName === user?.firstName ? 'items-end' : 'items-start'}`}>
                        <div className={`px-4 py-2 rounded-2xl max-w-[85%] ${msg.senderName === 'You' || msg.senderName === user?.firstName ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-slate-800 text-slate-200 rounded-bl-sm'}`}>
               ...
               <div className="p-4 bg-slate-900 border-t border-slate-800">
                  <div className="relative flex items-center">
                     <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && sendMessage()} placeholder="Type message..." className="w-full bg-slate-800 text-white rounded-xl pl-4 pr-12 py-3 border border-slate-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-sm"/>
                     <button onClick={sendMessage} className="absolute right-2 p-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 transition-colors"><Send size={16}/></button>
                  </div>
               </div>
             </>
           )}
           ...
           {showSidebar === 'participants' && (
             <>
               <div className="px-6 py-5 bg-slate-900 border-b border-slate-800">
                 <h3 className="font-bold text-white flex items-center gap-2"><Users size={18} className="text-indigo-400"/> Participants (2)</h3>
               </div>
               <div className="p-4 space-y-2">
                 <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl border border-slate-700/50">
                    <div className="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold">{user?.firstName?.[0] || 'Y'}</div>
                    <div className="flex-1"><p className="text-white text-sm font-bold">You <span className="text-xs bg-slate-700 px-2 py-0.5 rounded text-slate-300 ml-1">Patient</span></p></div>
                    <div className="flex gap-2 text-slate-400">{isMuted ? <MicOff size={14} className="text-red-400"/> : <Mic size={14}/>}</div>
                 </div>
                 <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl border border-slate-700/50">
                    <div className="w-10 h-10 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold">{sessionInfo?.doctorName?.[4] || 'D'}</div>
                    <div className="flex-1"><p className="text-white text-sm font-bold">{sessionInfo?.doctorName}</p><span className="text-xs text-emerald-400 font-medium">Host</span></div>
                    <div className="flex gap-2 text-slate-400"><Mic size={14}/></div>
                 </div>
               </div>
             </>
           )}
