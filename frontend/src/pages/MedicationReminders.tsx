    ...
    <div className="max-w-7xl mx-auto py-8 px-4 animate-slide-up">
           ...
           <div className="premium-glass p-6 sticky top-24">
              ...
              <div className="space-y-3">
                 {[ 
                   ...
                   <button 
                     ...
                     className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 ${filter === slot.id ? 'bg-indigo-600 border-indigo-600 text-slate-900 shadow-lg' : 'bg-white border-slate-100 text-slate-600 hover:border-indigo-200 hover:bg-indigo-50/30'}`}>
                      <div className="flex items-center gap-3">
                         <slot.icon size={20} className={filter === slot.id ? 'text-indigo-200' : 'text-slate-600'} />
                         <span className="font-bold">{slot.label}</span>
                      </div>
                      <span className={`px-2 py-0.5 rounded-lg text-xs font-black ${filter === slot.id ? 'bg-white/20 text-slate-900' : 'bg-slate-100 text-slate-500'}`}>
                         {slot.count}
                      </span>
                   </button>
                 ))}
              ...
              <div className="mt-10 pt-10 border-t border-slate-100 space-y-6">
                 <div className="flex justify-between items-end border-b border-slate-100 pb-4">
                    <p className="text-sm font-black uppercase tracking-widest text-slate-600">Streak</p>
                    <p className="text-3xl font-black text-emerald-600">12 Days</p>
                 </div>
                 <div className="flex justify-between items-end border-b border-slate-100 pb-4">
                    <p className="text-sm font-black uppercase tracking-widest text-slate-600">Total Taken</p>
             ...
             <div key={med.id} className="premium-glass p-8 group hover:border-indigo-200 transition-all cursor-pointer relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between gap-6">
                   <div className="flex gap-6">
                      <div className={`w-20 h-20 rounded-[2rem] ${med.color} flex items-center justify-center text-slate-900 shadow-lg shadow-black/5 shrink-0 group-hover:scale-105 transition-transform duration-500`}>
                         <Pill size={40} />
                      </div>
                      <div>
                   ...
                   <div className="flex md:flex-col justify-between items-end gap-3 min-w-[140px]">
                      {med.status === 'taken' ? (
                        <div className="flex flex-col items-end gap-1">
                           ...
                           <p className="text-xs font-medium text-slate-600">At 09:12 AM Today</p>
                        </div>
                      ) : (
                        <button className="flex-1 md:w-full py-4 bg-slate-900 text-slate-900 rounded-2xl font-black hover:bg-indigo-600 transition-all shadow-xl active:scale-95 group/btn">
                           TAKE DOSE <span className="group-hover:translate-x-1 inline-block transition-transform">→</span>
                        </button>
                      )}
                      
                      <div className="flex gap-2">
                         <button className="p-3 bg-slate-50 text-slate-600 rounded-xl hover:bg-slate-100 hover:text-slate-600 transition-all">
                            <Plus size={18} />
                         </button>
                         <button className="p-3 bg-slate-50 text-slate-600 rounded-xl hover:bg-slate-100 hover:text-red-500 transition-all">
                            <Trash2 size={18} />
                         </button>
                ...
                <div className="mt-8 pt-8 border-t border-slate-100 flex items-center gap-6">
                   ...
                   <div className="text-right">
                      <p className="text-[10px] font-black uppercase text-slate-600 tracking-widest mb-1">Stock Level</p>
      ...
      <div className="mt-20 grid md:grid-cols-3 gap-8">
         <div className="premium-glass p-8 bg-slate-900 text-slate-900 flex flex-col items-center text-center">
            ...
            <p className="text-slate-600 text-sm font-medium mb-6">Export your adherence history as a CSV or PDF for your next visit.</p>
            ...
            <div className="relative shrink-0">
               <div className="w-32 h-32 rounded-full border-8 border-indigo-100 flex items-center justify-center">
                  <div className="text-center">
                     ...
                     <p className="text-[10px] font-black text-slate-600 uppercase tracking-tighter">Adherence</p>
                  </div>
               </div>
