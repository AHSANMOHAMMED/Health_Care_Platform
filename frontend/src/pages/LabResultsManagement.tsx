                 ...
                 <div 
                   ...
                   className={`premium-glass p-6 cursor-pointer transition-all ${selectedReport === report.id ? 'border-2 border-indigo-500 bg-white ring-4 ring-indigo-50' : 'hover:border-indigo-200'}`}>
                    <div className="flex justify-between items-start mb-3">
                       ...
                       <p className="text-xs font-black text-slate-400 uppercase">{report.id}</p>
                    </div>
        ...
        <div className="lg:col-span-2 space-y-8">
           {selectedReport ? (
             <div className="premium-glass p-8 md:p-12 animate-in fade-in zoom-in-95 duration-500">
                   ...
                   <div className="flex gap-6">
                      ...
                      <div>
                         ...
                         <div className="flex gap-3 mt-3">
                            <span className="flex items-center gap-1.5 text-xs font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-full"><Calendar size={14}/> {reports.find(r => r.id === selectedReport)?.date}</span>
                ...
                <div className="space-y-4">
                   <div className="grid grid-cols-4 px-6 py-3 text-xs font-black uppercase tracking-widest text-slate-400">
                      ...
                      {reports.find(r => r.id === selectedReport)?.metrics.map(metric => (
                        <div key={metric.name} className="grid grid-cols-4 px-6 py-6 bg-slate-50 border border-slate-100 rounded-2xl group hover:bg-white hover:border-indigo-100 hover:shadow-soft transition-all">
                           <div className="col-span-2">
                              ...
                              <p className="text-xs font-bold text-slate-400">Biological Reference Interval</p>
                           </div>
                           <div className="text-right">
                              <p className={`text-2xl font-black ${metric.status === 'normal' ? 'text-slate-900' : 'text-red-500'}`}>{metric.value} <span className="text-sm font-medium text-slate-400 uppercase">{metric.unit}</span></p>
                           ...
                           <div className="text-right flex flex-col justify-center">
                              ...
                              <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">{metric.unit}</p>
                           </div>
                        </div>
                      ))}
                   </div>
                ...
                <div className="mt-12 bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden">
                   ...
                   <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-xl relative z-10">All markers in this CMP panel are within optimal ranges. Your kidney filtration rate (Creatinine) is excellent. Continue current hydration level.</p>
