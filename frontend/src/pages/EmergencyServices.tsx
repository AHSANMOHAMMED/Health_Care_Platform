...
export default function EmergencyServices() {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-50 flex items-center justify-center p-6">
          <div className="bg-red-950 border-2 border-red-500 rounded-3xl p-10 max-w-lg w-full text-center">
            <div className="w-24 h-24 bg-red-600 rounded-full flex items-center justify-center mx-auto mb-6 animate-bounce shadow-2xl shadow-red-900/60">
              <Ambulance size={48} className="text-slate-900" />
            </div>
            ...
            <p className="text-slate-600 font-medium text-sm mb-8">Your emergency contacts have been notified. Medical history pre-shared with paramedics. Stay where you are.</p>
            <div className="flex gap-3">
              <a href="tel:1990" className="flex-1 py-4 bg-red-600 text-slate-900 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-red-500">
                <Phone size={20} /> Call 1990
              </a>
              <button onClick={() => setSosState('idle')} className="flex-1 py-4 bg-white/10 border border-white/20 text-slate-900 rounded-2xl font-bold hover:bg-white/20">
                Cancel SOS
              </button>
            </div>
          </div>
          ...
          {sosState === 'idle' && (
            <div className="flex flex-col lg:flex-row items-center gap-10">
              <div className="flex-1">
                ...
                <h1 className="text-4xl lg:text-[#0284C7]xl font-black text-slate-900 tracking-tighter mb-4">
                  Need Immediate Help?
                </h1>
                <p className="text-slate-600 font-medium text-lg leading-relaxed mb-8 max-w-lg">
              ...
              <div className="flex flex-col items-center gap-4">
                <button
                  onClick={handleSOS}
                  disabled={activating}
                  className="relative w-44 h-44 rounded-full bg-gradient-to-br from-red-600 to-red-800 text-slate-900 flex flex-col items-center justify-center shadow-2xl shadow-red-900/60 hover:scale-105 active:scale-95 transition-all border-8 border-red-950/50 group"
                >
          ...
          {sosState === 'confirming' && (
            <div className="text-center py-6">
              <div className="w-24 h-24 rounded-full bg-red-600/20 border-2 border-red-500 flex items-center justify-center mx-auto mb-6">
                <span className="text-[#0284C7]xl font-black text-red-400">{countdown}</span>
              </div>
              <h2 className="text-3xl font-black text-red-300 mb-3">Dispatching in {countdown}s</h2>
              <p className="text-slate-600 font-medium mb-6">Suwa Seriya will be alerted automatically. Cancel if this was accidental.</p>
              <div className="flex gap-3 max-w-sm mx-auto">
                <a href="tel:1990" className="flex-1 py-4 bg-red-600 text-slate-900 rounded-2xl font-black flex items-center justify-center gap-2 text-sm hover:bg-red-500">
                  <PhoneCall size={18} /> Call Now
                </a>
                <button onClick={() => setSosState('idle')} className="flex-1 py-4 bg-white/10 border border-white/20 text-slate-900 rounded-2xl font-bold text-sm hover:bg-white/20">
      ...
      {/* ── EMERGENCY HOTLINES ── */}
      <div>
        <h2 className="text-2xl font-black text-slate-900 tracking-tight mb-5">Emergency Hotlines</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {EMERGENCY_NUMBERS.map(({ service, number, color, icon: Icon, desc }) => (
            <a key={number} href={`tel:${number}`}
              ...
              <Icon size={28} className="text-slate-900 mb-3 group-hover:scale-110 transition-transform" />
              <p className="text-2xl font-black text-slate-900 mb-1">{number}</p>
              <p className="font-black text-slate-900 text-sm leading-tight">{service}</p>
              <p className="text-slate-900/60 text-[10px] font-medium mt-1">{desc}</p>
            </a>
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-5 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setLocating(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-600 hover:text-slate-900 hover:bg-white/10 transition-all text-sm font-bold">
        ...
        <div className="relative mb-5">
          ...
          <input type="text" placeholder="Search by name, specialty, area..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full pl-12 pr-4 py-4 bg-white/5 border border-white/10 rounded-2xl text-slate-900 placeholder-slate-500 font-medium text-sm focus:outline-none focus:border-white/20 transition-all" />
        </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <h3 className="font-black text-slate-900 text-lg truncate">{h.name}</h3>
                ...
                <div className="flex gap-2 lg:flex-col flex-shrink-0">
                  <a href={`tel:${h.phone}`}
                    className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-red-600/80 text-slate-900 text-sm font-black hover:bg-red-600 transition-all shadow-lg shadow-red-900/30">
                    <Phone size={16} /> Call ER
                  </a>
                  <a href={`https://maps.google.com/?q=${h.name} ${h.address}`} target="_blank" rel="noreferrer"
                    className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-white/5 border border-white/10 text-slate-900 text-sm font-bold hover:bg-white/10 transition-all">
      ...
      {/* ── FIRST AID GUIDE ── */}
      <div>
        <h2 className="text-2xl font-black text-slate-900 tracking-tight mb-2">First Aid Quick Guide</h2>
            ...
            <div key={i}
              ...
              <div className="flex items-center gap-4 p-6">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all ${activeTip === i ? 'bg-amber-500' : 'bg-white/5 border border-white/5'}`}>
                  <tip.icon size={22} className={activeTip === i ? 'text-slate-900' : 'text-slate-600'} />
                </div>
                <div className="flex-1">
